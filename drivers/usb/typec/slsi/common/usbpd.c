...
void usbpd_crc_timer_start(struct usbpd_data *pd_data)
{
	ktime_get_ts64(&pd_data->crc_time);
}

int usbpd_check_crc_timer(struct usbpd_data *pd_data)
{
	uint32_t ms = 0;
	uint32_t sec = 0;
	struct timespec64 time;
	...
	sec = time.tv_sec - pd_data->crc_time.tv_sec;
	ms = (time.tv_nsec - pd_data->crc_time.tv_nsec) / 1000000;
...
protocol_state usbpd_protocol_tx_wait_for_message_request(struct protocol_data *tx)
{
...
void usbpd_set_ops(struct device *dev, usbpd_phy_ops_type *ops)
{
	...
	pd_data->phy_ops.set_revision			= ops->set_revision;
	pd_data->phy_ops.get_rp_level			= ops->get_rp_level;
	pd_data->phy_ops.ops_set_clk_offset = ops->ops_set_clk_offset;
	pd_data->phy_ops.ops_check_pps_irq_reduce_clk = ops->ops_check_pps_irq_reduce_clk;
...
bool usbpd_receive_msg(struct usbpd_data *pd_data)
{
	usbpd_protocol_rx(pd_data);

	if (pd_data->protocol_rx.status == MSG_RECEIVED)
		return true;
	else
		return false;
}
EXPORT_SYMBOL(usbpd_receive_msg);
		...
		enum pdic_sysfs_property prop, char *buf)
{
	...
	if (!manager) {
		usbpd_err("%s manager_data is null!!\n", __func__);
		return -ENODEV;
	}
				...
				const char *buf, size_t size)
{
	...
	case PDIC_SYSFS_PROP_SET_WATER_THRESHOLD:
		...
		usbpd_info("%s, PROP_SET_WATER_THRESHOLD buf : %s\n", __func__, buf);
		usbpd_info("%s, #%d is set to %d\n", __func__, val1, val2);
		PDIC_OPS_PARAM2_FUNC(ops_set_water_threshold, pd_data, val1, val2);
#endif
		break;
	case PDIC_SYSFS_PROP_CTRL_OPTION:
		sscanf(buf, "%d", &cmd);
		usbpd_info("usb: %s PROP_CTRL_OPTION mode=%d\n", __func__, cmd);
...
int usbpd_init(struct device *dev, void *phy_driver_data)
{
	...
	pd_data = devm_kzalloc(dev, sizeof(struct usbpd_data), GFP_KERNEL);
	...
	mutex_init(&pd_data->softreset_mutex);
	...
	if (!pd_data->policy_wqueue)
		usbpd_err("%s: Fail to Create Workqueue\n", __func__);
	...
	if (!pd_data->pdic_wq) {
		usbpd_err("%s failed to create work queue for pdic notifier\n",
			__func__);
		return -ENOMEM;
	}
	ret = pdic_misc_init(ppdic_data);
	if (ret) {
		usbpd_err("pdic_misc_init is failed, error %d\n", ret);
	} else {
