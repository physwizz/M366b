...
static int set_alpm_mode_factory(struct panel_device *panel, int mode)
{
	int ret = 0;
	int lpm_mode = (mode & 0xFF);
	static int backup_br[MAX_PANEL_DEV];
	struct panel_info *panel_data = &panel->panel_data;
	struct panel_bl_device *panel_bl = &panel->panel_bl;
	struct backlight_device *bd = panel_bl->bd;
#if defined(CONFIG_USDM_PANEL_AOD_BL)
	...
	case ALPM_OFF:
		...
		if (backup_br[panel->id])
			bd->props.brightness = backup_br[panel->id];
	...
	case HLPM_HIGH_BR:
		panel_data->props.alpm_mode = lpm_mode;
		backup_br[panel->id] = bd->props.brightness;
#if defined(CONFIG_USDM_PANEL_AOD_BL)
		if (lpm_ver == 0) {
			bd->props.brightness =
				(lpm_mode <= HLPM_LOW_BR) ? BRT(0) : BRT(94);
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", panel_bl->props.night_dim_range);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_bl_set_property(panel_bl, &panel_bl->props.night_dim_range, value);
	panel_mutex_unlock(&panel_bl->lock);
	panel_update_brightness(panel);
...
bool panel_replace_svc_octa_node_name(struct panel_device *panel, struct panel_device_attr *attr, struct panel_device_attr_replace *replace_attr)
{
	if (!panel || !attr || !replace_attr)
		return false;

	if (!replace_attr->from || !replace_attr->to) {
		panel_err("got invalid replace_attr(from:%s, to:%s)\n", replace_attr->from, replace_attr->to);
		return false;
	}
	if (!strcmp(attr->dev_attr.attr.name, replace_attr->from)) {
		if (panel->id == 1) {
			panel_info("replace sysfs %s to %s\n", attr->dev_attr.attr.name, replace_attr->to);
			attr->dev_attr.attr.name = replace_attr->to;
			return true;
		}
	}
...
struct panel_device_attr_replace panel_sysfs_replace_attrs[] = {
	{
		.replace = panel_replace_svc_octa_node_name,
		.from = "SVC_OCTA",
		.to = "SVC_OCTA2",
	},
	{
		.replace = panel_replace_svc_octa_node_name,
		.from = "SVC_OCTA_CHIPID",
		.to = "SVC_OCTA2_CHIPID",
	},
	{
		.replace = panel_replace_svc_octa_node_name,
		.from = "SVC_OCTA_DDI_CHIPID",
		.to = "SVC_OCTA2_DDI_CHIPID",
	},
};
...
void panel_replace_sysfs_node(struct panel_device *panel, struct panel_device_attr *attr)
{
	int k;

	for (k = 0; k < ARRAY_SIZE(panel_sysfs_replace_attrs); k++) {
		if (!panel_sysfs_replace_attrs[k].replace)
			continue;

		if (panel_sysfs_replace_attrs[k].replace(panel, attr, &panel_sysfs_replace_attrs[k]))
			return;
	}
}
int panel_create_sysfs(struct panel_device *panel)
{
	...
	for (i = 0; i < ARRAY_SIZE(panel_attrs); i++) {
		...
		panel_replace_sysfs_node(panel, &(panel_attrs[i]));
