...
#define MAX_SLEEP_OUT_RETRY_CNT (3)
	...
	if (!list_empty(&panel_device_list)) {
		list_for_each_entry(panel, &panel_device_list, list) {
			...
			if (panel->id < 0 || panel->id >= MAX_PANEL_DEV) {
				panel_err("panel id %d out of range\n", panel->id);
				continue;
			}
			...
			if (strncmp(lut_info->name, info->name, 128)) {
				panel_dbg("name mismatched. %s:%s\n", lut_info->name, info->name);
				continue;
			}
...
__visible_for_testing const char *panel_get_bypass_reason_str(struct panel_device *panel)
{
	static const char *str_bypass_reason[MAX_BYPASS_REASON] = {
...
static void panel_check_evasion_disp_det_support(struct panel_device *panel)
{
	...
	panel->panel_irq_func_table[PANEL_GPIO_DISP_DET].enable = panel_enable_evasion_disp_det_irq;
	panel->panel_irq_func_table[PANEL_GPIO_DISP_DET].disable = panel_disable_evasion_disp_det_irq;
...
int panel_enable_irq(struct panel_device *panel, u32 pins)
{
	...
	for (panel_gpio_id = 0; panel_gpio_id < PANEL_GPIO_MAX; panel_gpio_id++) {
		...
		if (!panel_is_gpio_valid(panel, panel_gpio_id)) {
			if (panel->irq_invalid_log_once_flag & (0x1 << panel_gpio_id)) {
				panel_info("%s is not supported.\n", panel_gpio_names[panel_gpio_id]);
				panel->irq_invalid_log_once_flag = panel->irq_invalid_log_once_flag & ~(BIT_MASK(panel_gpio_id));
			}
			continue;
		}
		...
		if (panel->panel_irq_func_table[panel_gpio_id].enable)
			ret = panel->panel_irq_func_table[panel_gpio_id].enable(panel);
		else
...
int panel_disable_irq(struct panel_device *panel, u32 pins)
{
	...
	for (panel_gpio_id = PANEL_GPIO_MAX - 1 ; panel_gpio_id >= 0; panel_gpio_id--) {
		...
		if (panel->panel_irq_func_table[panel_gpio_id].disable)
			ret = panel->panel_irq_func_table[panel_gpio_id].disable(panel);
		else
...
static inline void panel_send_esd_recovery_notify(int display_idx, u32 state)
{
	struct panel_notifier_event_data evt_data = {
		.display_index = 0,
		.state = state,
	...
	evt_data.display_index = display_idx;
	panel_notifier_call_chain(PANEL_EVENT_ESD_STATE_CHANGED, &evt_data);
	panel_info("notify PANEL_EVENT_ESD_STATE_CHANGED (state:%d)\n", evt_data.state);
}
...
int panel_default_ssr_test(struct panel_device *panel)
{
	...
	ret = panel_do_seqtbl_by_name(panel, PANEL_SSR_TEST_SEQ);
	if (unlikely(ret < 0)) {
		panel_err("failed to run sequence(%s)\n", PANEL_SSR_TEST_SEQ);
		return ret;
	}
...
static void panel_update_handler(struct work_struct *work)
{
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF ||
		panel_get_cur_state(panel) == PANEL_STATE_ON) {
		panel_info("state %d, skip update\n", panel_get_cur_state(panel));
		goto exit;
	}
...
int panel_vote_up_to_probe(struct panel_device *panel, struct common_panel_info *info)
{
	struct panel_device *match_panel_drv = NULL, *panel_itr = NULL;
	static int vote_up_count[MAX_PANEL_DEV];
	static int need_vote_up_count[MAX_PANEL_DEV];
	struct panel_dt_lut *lut_info = NULL;
	bool found = false;

	/*
	 * The common_panel_info don't know  which panel-drv belongs to.
	 * Find a Panel-drv who has lut-info matching to cpi.
	 * The found Panel-drv  is for common_panel_info.
	 */
	...
	if (list_empty(&panel_device_list)) {
		mutex_unlock(&panel_device_list_lock);
		panel_err("panel_device_list empty.\n");
		return -EINVAL;
	}

	list_for_each_entry(panel_itr, &panel_device_list, list) {
		list_for_each_entry(lut_info, &panel_itr->panel_lut_list, head) {
			if (!info)
				continue;

			if (IS_ERR_OR_NULL(lut_info)) {
				panel_warn("Null lut_info in %s\n", panel_itr->of_node_name);
				continue;
			}
			...
			if (!strncmp(lut_info->name, info->name, 128)) {
				match_panel_drv = panel_itr;
				found = true;
				panel_info("%s:found(panel-%d)(%s)(%s)\n", __func__, panel_itr->id, lut_info->name, info->name);
			}
		}
	}
	...
	if (!match_panel_drv && panel)
		match_panel_drv = panel;

	if (!match_panel_drv) {
		panel_err("There is no match between CPI and Panel_lut.\n");
		return -EAGAIN;
	}

	vote_up_count[match_panel_drv->id]++;

	panel_info("vote_up_count(%d)\n", vote_up_count[match_panel_drv->id]);

	if (!need_vote_up_count[match_panel_drv->id])
		need_vote_up_count[match_panel_drv->id] = panel_get_need_vote_up_count(match_panel_drv);

	if (vote_up_count[match_panel_drv->id] >= need_vote_up_count[match_panel_drv->id]) {
		panel_info("probe start! vote_up_count(%d) need_vote_up_count(%d)\n",
				vote_up_count[match_panel_drv->id], need_vote_up_count[match_panel_drv->id]);
		panel_probe(match_panel_drv);
		return 0;
	}
...
__visible_for_testing int panel_sleep_out(struct panel_device *panel)
{
	...
	case PANEL_STATE_ON:
		...
		if (ret) {
			if (--panel->sleep_out_retry >= 0 && ret == -EAGAIN) {
				panel_dsi_set_bypass(panel, true);
		...
		panel->sleep_out_retry = MAX_SLEEP_OUT_RETRY_CNT;
		break;
	default:
		panel_err("invalid state(%d)\n", panel_get_cur_state(panel));
		goto do_exit;
...
int panel_vrr_cb(struct panel_device *panel)
{
	...
	static struct panel_event_dms_data old_dms_data[MAX_PANEL_DEV];
#endif
	struct panel_properties *props;
	...
	if (old_dms_data[panel->id].fps != dms_data.fps) {
	...
	if (memcmp(&old_dms_data[panel->id], &dms_data, sizeof(dms_data))) {
	...
	memcpy(&old_dms_data[panel->id], &dms_data, sizeof(dms_data));
#endif
...
int panel_ioctl_event_frame_done(struct panel_device *panel, void *arg)
{
	...
	static int mres_updated_frame_cnt[MAX_PANEL_DEV];
#endif
	...
	if (panel->panel_data.props.mres_updated &&
			(++mres_updated_frame_cnt[panel->id] > 1)) {
		panel_set_property(panel, &panel->panel_data.props.mres_updated, false);
		mres_updated_frame_cnt[panel->id] = 0;
	}
#endif
...
static void disp_det_handler(struct work_struct *work)
{
	...
	panel_send_esd_recovery_notify(panel->id, 0);
...
void panel_send_ubconn_uevent(struct panel_device *panel)
{
	...
	char *uevent_conn_str_sub[3] = {
		"CONNECTOR_NAME=UB_CONNECT_SUB",
		"CONNECTOR_TYPE=HIGH_LEVEL",
		NULL,
	};
	int id = 0;
	...
	if (id == 0) {
		...
		kobject_uevent_env(&panel->lcd_dev->kobj, KOBJ_CHANGE, uevent_conn_str_sub);
		panel_info("%s, %s\n", uevent_conn_str_sub[0], uevent_conn_str_sub[1]);
	}
}
				 ...
				 unsigned long event, void *data)
{
	...
	set_dpui_field((panel->id == 1) ? DPUI_KEY_SUB_CELLID : DPUI_KEY_CELLID, tbuf, size);

	/* OCTAID */
	ret = panel_get_octa_id(panel, tbuf);
	if (ret > 0)
		set_dpui_field((panel->id == 1) ? DPUI_KEY_SUB_OCTAID : DPUI_KEY_OCTAID, tbuf, ret - 1);

	/* CHIPID */
	ret = panel_get_manufacture_code(panel, tbuf);
	if (ret == 0 && strlen(tbuf) > 0)
		set_dpui_field((panel->id == 1) ? DPUI_KEY_SUB_CHIPID : DPUI_KEY_CHIPID, tbuf, strlen(tbuf) - 1);
...
int panel_device_init(struct panel_device *panel)
{
	...
	panel->sleep_out_retry = MAX_SLEEP_OUT_RETRY_CNT;
	panel->irq_invalid_log_once_flag = PANEL_IRQ_ALL;
...
#else
static void panel_drv_remove(struct platform_device *pdev)
{
	struct panel_device *panel;
