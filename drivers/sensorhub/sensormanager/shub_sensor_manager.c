...
struct init_func_t init_sensor_funcs[] = {
	...
	{SENSOR_TYPE_BACK_TAP, init_back_tap},
};
...
uint32_t get_foldable_type(void)
{
	return sensor_manager->foldable_type;
}
...
static void init_foldable_type(void)
{
	struct device_node *np = get_shub_device()->of_node;

	of_property_read_u32(np, "foldable-type", &sensor_manager->foldable_type);

	shub_infof("foldable-type : %d", sensor_manager->foldable_type);
}
