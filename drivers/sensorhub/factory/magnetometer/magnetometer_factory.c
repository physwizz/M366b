...
get_chipset_funcs_ptr get_magnetometer_factory_chipset_funcs_ptr[] = {
	...
	get_magnetometer_ak09920c_chipset_func,
...
static ssize_t flip_matrix_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	if (!chipset_func ||!chipset_func->flip_matrix_store)
		return -EINVAL;

	return chipset_func->flip_matrix_store(buf, size);
}

static ssize_t flip_matrix_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!chipset_func ||!chipset_func->flip_matrix_show)
		return -EINVAL;

	return chipset_func->flip_matrix_show(buf);
}
...
static DEVICE_ATTR(flip_matrix, 0664, flip_matrix_show, flip_matrix_store);
...
void remove_magnetometer_empty_sysfs(void)
{
	...
	if (!data->flip_matrix) {
		shub_errf("flip_matrix is null");
		device_remove_file(mag_sysfs_device, &dev_attr_flip_matrix);
	}
