...
get_init_chipset_funcs_ptr get_mag_funcs_ary[] = {
	...
	get_magnetic_ak09920c_function_pointer,
...
static int init_magnetometer_variable(int type)
{
	...
	if (data->cal_data_len) {
		if (get_foldable_type())
			data->cal_data_len = data->cal_data_len * 2;
	...
	if (data->mag_matrix_len) {
		...
		data->flip_matrix = kzalloc(data->mag_matrix_len, GFP_KERNEL);
		if (!data->flip_matrix)
			return -ENOMEM;
...
int set_mag_flip_matrix(struct magnetometer_data *data)
{
	int ret = 0;
	...
	ret = shub_send_command(CMD_SETVALUE, SENSOR_TYPE_GEOMAGNETIC_FIELD, MAG_SUBCMD_FLIP_MATRIX,
				(char *)data->flip_matrix, data->mag_matrix_len);
	shub_infof("%u", data->position);
...
static int save_mag_calibration_file(void)
{
	...
	shub_infof("%d, %d %d %d, %d %d %d, %d %d %d", cal_data->accuracy,
		    cal_data->offset_x, cal_data->offset_y, cal_data->offset_z,
		    cal_data->flucv_x, cal_data->flucv_y, cal_data->flucv_z,
		    cal_data->base_x, cal_data->base_y, cal_data->base_z);

	if (get_foldable_type()) {
		struct calibration_data_ak09918c *folding_cal_data = data->cal_data + data->cal_data_len/2;

		shub_infof("%d, %d %d %d, %d %d %d, %d %d %d", folding_cal_data->accuracy,
			    folding_cal_data->offset_x, folding_cal_data->offset_y, folding_cal_data->offset_z,
			    folding_cal_data->flucv_x, folding_cal_data->flucv_y, folding_cal_data->flucv_z,
			    folding_cal_data->base_x, folding_cal_data->base_y, folding_cal_data->base_z);
	}
...
static int set_mag_cal(struct magnetometer_data *data)
{
	...
	if (get_foldable_type()) {
		...
		ret = shub_send_command(CMD_SETVALUE, SENSOR_TYPE_GEOMAGNETIC_FIELD, CAL_DATA,
					(char *)data->cal_data, data->cal_data_len/2);
		ret = shub_send_command(CMD_SETVALUE, SENSOR_TYPE_GEOMAGNETIC_FIELD, CAL_DATA,
					(char *)(data->cal_data + (data->cal_data_len/2)), data->cal_data_len/2);
...
int init_magnetometer(bool en)
{
	...
	if (en) {
		...
		kfree_and_clear(magnetometer_data.flip_matrix);
