		...
		struct p61_ioctl_transfer *tr)
{
	...
	if (tr->len > MAX_BUFFER_SIZE || tr->len <= 0)
		return -EMSGSIZE;
	...
	if (tr->rx_buffer != NULL) {
		if (tr->len == 2 && ((p61_dev->r_buf[0] == 0x0 && p61_dev->r_buf[1] == 0x0) ||
				(p61_dev->r_buf[0] == 0xff && p61_dev->r_buf[1] == 0xff))) {
			read_try_cnt++;
		} else {
			if (read_try_cnt)
...
#else
	geni_mas = spi_controller_get_devdata(master);
#endif
...
void p61_parse_pinctrl_set_nc(struct device *dev)
{
	struct device_node *spi_dev_node;
	struct device_node *np = dev->of_node;
	struct platform_device *spi_pdev;
	...
	pinctrl_spi = devm_pinctrl_get_select(&spi_pdev->dev, "ese_spi_nc");
	if (IS_ERR_OR_NULL(pinctrl_spi)) {
		NFC_LOG_ERR("Failed to pinctrl ese_spi_nc\n");
	} else {
		devm_pinctrl_put(pinctrl_spi);
		NFC_LOG_ERR("ese_spi_nc pinctrl called\n");
	}

	pinctrl = devm_pinctrl_get_select(dev, "ese_gpio_nc");
	if (IS_ERR_OR_NULL(pinctrl)) {
		NFC_LOG_ERR("Failed to pinctrl ese_gpio_nc\n");
	} else {
		devm_pinctrl_put(pinctrl);
		NFC_LOG_ERR("ese_gpio_nc pinctrl called\n");
	}
...
#else
static void p61_platform_remove(struct platform_device *pdev)
#endif
{
	NFC_LOG_INFO("Entry : %s\n", __func__);
#if (KERNEL_VERSION(6, 12, 0) > LINUX_VERSION_CODE)
