		 ...
		 uint8_t interface)
{
...
#if IS_ENABLED(CONFIG_SAMSUNG_NFC)
	nfc_det_gpio = of_get_named_gpio(np, DTS_DET_GPIO_STR, 0);
	if (!gpio_is_valid(nfc_det_gpio)) {
		NFC_LOG_INFO("%s : nfc-det-gpio is not set\n", __func__);
	} else {
		ret = gpio_request(nfc_det_gpio, "nfc_det_gpio");
		if (ret < 0)
			NFC_LOG_ERR("%s failed to get gpio nfc_det_gpio\n", __func__);
	...
	if (of_property_read_bool(np, "sec-nfc,clk_req_eint_mode")) {
		nfc_configs->clk_req_eint_mode = true;
		esca_init_eint_nfc_clk_req(nfc_gpio->clk_req);
		NFC_LOG_INFO("clk_req_eint_mode\n");
	}
#endif
	...
	nfc_configs->separated_se_vdd = of_property_read_bool(np, "pn547,separated_se_vdd");
	if (nfc_configs->separated_se_vdd) {
		nfc_configs->nfc_se_vdd = regulator_get(dev, "nfc_se_vdd");
		if (IS_ERR(nfc_configs->nfc_se_vdd))
			NFC_LOG_ERR("nfc_se_vdd not support\n");
		else
			NFC_LOG_INFO("LDO nfc_se_vdd: %pK, vol:%d\n",
				nfc_configs->nfc_se_vdd, regulator_get_voltage(nfc_configs->nfc_se_vdd));
	}
...
int nfc_regulator_onoff(struct nfc_dev *nfc_dev, int onoff)
{
	...
	NFC_LOG_INFO("pvdd onoff = %d, g_is_nfc_pvdd_enabled = %d\n",
	...
	if (nfc_configs->separated_se_vdd && IS_ERR_OR_NULL(nfc_configs->nfc_se_vdd)) {
		if (nfc_dev->i2c_dev.client) {
			nfc_configs->nfc_se_vdd = regulator_get(&nfc_dev->i2c_dev.client->dev, "nfc_se_vdd");
			if (IS_ERR(nfc_configs->nfc_se_vdd))
				NFC_LOG_ERR("nfc_se_vdd not support\n");
			else
				NFC_LOG_INFO("LDO nfc_se_vdd: ok, vol:%d\n",
					regulator_get_voltage(nfc_configs->nfc_se_vdd));
		} else {
			NFC_LOG_ERR("i2c_dev.client not ready\n");
		}
	}
	...
	if (onoff) {
		...
		if (!IS_ERR_OR_NULL(nfc_configs->nfc_se_vdd)) {
			rc = regulator_enable(nfc_configs->nfc_se_vdd);
			if (rc) {
				NFC_LOG_ERR("se vdd enable failed, rc=%d\n", rc);
				goto done;
			}
		}
	} else {
		if (!IS_ERR_OR_NULL(nfc_configs->nfc_se_vdd)) {
			rc = regulator_disable(nfc_configs->nfc_se_vdd);
			if (rc) {
				NFC_LOG_ERR("se vdd disable failed, rc=%d\n", rc);
				goto done;
			}
		}
