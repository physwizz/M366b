...
int dev_ril_bridge_send_msg(int id, int size, void *buf)
{
	...
	if (id > IPC_SYSTEM_CP_CMD_LAST) {
		drb_err("ERR! wrong id\n");
		return -EINVAL;
	}
...
static BLOCKING_NOTIFIER_HEAD(dev_ril_bridge_chain);
...
int register_dev_ril_bridge_event_notifier(struct notifier_block *nb)
{
	...
	return blocking_notifier_chain_register(&dev_ril_bridge_chain, nb);
}
...
int unregister_dev_ril_bridge_event_notifier(struct notifier_block *nb)
{
	...
	return blocking_notifier_chain_unregister(&dev_ril_bridge_chain, nb);
}
			...
			size_t count, loff_t *fops)
{
	...
	blocking_notifier_call_chain(&dev_ril_bridge_chain,
