...
static int aw_dev_reg_fw_update(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < data_len; i += 2) {
		...
		if (reg_addr == aw_dev->psm_desc.reg) {
			aw_dev->psm_desc.reserve_val = reg_val & (~aw_dev->psm_desc.mask);
			aw_dev_info(aw_dev->dev, "reserve_val = 0x%04x",
						aw_dev->psm_desc.reserve_val);
		}
		if (reg_addr == aw_dev->mpd_desc.reg) {
			aw_dev->mpd_desc.reserve_val = reg_val & (~aw_dev->mpd_desc.mask);
			aw_dev_info(aw_dev->dev, "reserve_val = 0x%04x",
						aw_dev->mpd_desc.reserve_val);
		}
		if (reg_addr == aw_dev->dsmzth_desc.reg) {
			aw_dev->dsmzth_desc.reserve_val = reg_val & (~aw_dev->dsmzth_desc.mask);
			aw_dev_info(aw_dev->dev, "reserve_val = 0x%04x",
						aw_dev->dsmzth_desc.reserve_val);
		}
...
void aw882xx_dev_mute(struct aw_device *aw_dev, bool mute)
{
	...
	if (aw_dev->mute_flag) {
		mute_flag = aw_dev->mute_flag;
		aw_dev_info(aw_dev->dev, "enter, mute_flag is true, force mute: %d, cali_result: %d",
					mute_flag, aw_dev->cali_desc.cali_result);
	} else {
...
static int aw_dev_mode1_pll_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev, "iis_abnormal_cnt[%d]", aw_dev->bd_record.iis_abnormal_cnt);
...
static int aw_dev_i2senable_check(struct aw_device *aw_dev)
{
	unsigned int reg_val = 0;
	struct aw_i2sen_desc *i2sen_desc = &aw_dev->i2sen_desc;

	aw_dev->ops.aw_i2c_read(aw_dev, i2sen_desc->reg, &reg_val);
	if ((reg_val & i2sen_desc->i2sen_check) != i2sen_desc->i2sen_check) {
		aw_dev_err(aw_dev->dev, "check i2sen fail with reg_val[0x%x] check_val[0x%x]",
			reg_val, i2sen_desc->i2sen_check);
		return -EINVAL;
	}

	if ((reg_val & i2sen_desc->i2srxen_check) != i2sen_desc->i2srxen_check) {
		aw_dev_err(aw_dev->dev, "check i2rxsen fail with reg_val[0x%x] check_val[0x%x]",
			reg_val, i2sen_desc->i2sen_check);
		return -EINVAL;
	}

	aw_dev_info(aw_dev->dev, "check i2sen&i2rxsen done");
	return 0;
}

static int aw_dev_sysctrl_mute_check_recovery(struct aw_device *aw_dev)
{
	int ret = -1;
	unsigned int reg_val = 0;
	struct aw_mute_desc *mute_desc = &aw_dev->mute_desc;

	ret = aw_dev->ops.aw_i2c_read(aw_dev, mute_desc->reg, &reg_val);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read reg_addr[0x%x] failed, can't check reg_val",
			mute_desc->reg);
		return -EINVAL;
	}
	if ((reg_val & mute_desc->pwd_check)) {
		aw_dev_err(aw_dev->dev, "check pwd fail with reg_val[0x%x] check_val[0x%x], try to recovery",
			reg_val, mute_desc->pwd_check);
		/*power on*/
		aw_dev_pwd(aw_dev, false);
		usleep_range(AW_2000_US, AW_2000_US + 10);
	}
	if ((reg_val & mute_desc->amppd_check)) {
		aw_dev_err(aw_dev->dev, "check amppd fail with reg_val[0x%x] check_val[0x%x], try to recovery",
			reg_val, mute_desc->amppd_check);
		/*amppd on*/
		aw_dev_amppd(aw_dev, false);
		usleep_range(AW_1000_US, AW_1000_US + 50);
	}
	if ((reg_val & mute_desc->hmute_check)) {
		aw_dev_err(aw_dev->dev, "check hmute fail with reg_val[0x%x] check_val[0x%x], try to recovery",
			reg_val, mute_desc->hmute_check);
		if (!aw_dev->mute_st)
			aw882xx_dev_mute(aw_dev, false);
		else
			aw_dev_info(aw_dev->dev, "keep hmute due to hmute is enabled by register config");
	}

	aw_dev_info(aw_dev->dev, "check sysctrl mute done");
	return 0;
}
...
int aw882xx_dev_init_cali_te(struct aw_device *aw_dev)
{
	...
	ret = aw882xx_cali_read_te_from_nvram(&cali_desc->cali_te, aw_dev->channel);
	if (ret) {
		aw_dev_info(aw_dev->dev, "read nvram cali failed, use default te");
		cali_desc->cali_te = AW_ERRO_CALI_VALUE;
		return 0;
	}
	return ret;
}
...
int aw882xx_device_init(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	aw_dev->pilot_tone_threshold_update = 0;
	aw_dev->pilot_tone_threshold_value = -1;
	aw_dev->mute_count = 0;
	aw_dev->mute_flag = false;
...
int aw_dev_set_sys_property(struct aw_device *aw_dev)
{
	int ret = -1;
	int sys_te = 0;
	int vol = 0;
	...
	ret = aw_update_sys_temp(aw_dev, &sys_te);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get temperature from system failed!");
		return ret;
	}

	ret = aw_dsp_write_ambient_temperature(aw_dev, &sys_te);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "write system temperature failed");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "update sys temperature %d done", sys_te);

	ret = aw_update_voltage_current(aw_dev, &vol, &cur);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "update_voltage_current failed");
		return ret;
	}

	ret = aw882xx_dsp_write_bat_property(aw_dev, &vol, &cur);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "write bat property failed");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "set bat property done!");

	return ret;
}

int aw_dev_get_init_data(struct aw_device *aw_dev, struct aw_init_data *init_data, unsigned int dev_cnt)
{
	int ret = -1;
	int sys_te = 0;
	int vol = 0;
	...
	if (ret)
		aw_dev_err(aw_dev->dev, "get temperature from system failed!");
	init_data->env_te = (sys_te << AW_AMBIENT_TEMP_ACCURACY) / 10;

	ret = aw_update_voltage_current(aw_dev, &vol, &cur);
	if (ret < 0)
		aw_dev_err(aw_dev->dev, "update_voltage_current failed");
	init_data->bat_voltage = vol;
	init_data->bat_current = cur;

	if (aw_dev->channel > AW_DEV_CH_PRI_R) {
		aw_dev_err(aw_dev->dev, "channel %d out of range for init data!", aw_dev->channel);
		return -EINVAL;
	}
	init_data->cali_re[aw_dev->channel] = AW_SHOW_RE_TO_DSP_RE(desc->cali_re);
	init_data->cali_te[aw_dev->channel] = (desc->cali_te << AW_CALI_TEMP_ACCURACY) / 10;
	...
	init_data->offset[aw_dev->channel] = offset;

	if (aw_dev->channel == dev_cnt - 1) {
		aw_dev_info(aw_dev->dev,
			"init env_te[%d] bat_voltage[%d] bat_current[%d] cali_re_l[%d] cali_re_r[%d] cali_te_l[%d] cali_te_r[%d] offset_l[%d] offset_r[%d]",
			init_data->env_te, init_data->bat_voltage, init_data->bat_current, init_data->cali_re[0], init_data->cali_re[1],
			init_data->cali_te[0], init_data->cali_te[1], init_data->offset[0], init_data->offset[1]);
	}
...
int aw882xx_device_start(struct aw_device *aw_dev)
{
	...
	ret = aw_dev_i2senable_check(aw_dev);
	if (ret < 0) {
		ret = aw882xx_dev_reg_update(aw_dev, true);
		if (ret) {
			aw_dev_err(aw_dev->dev, "fw update failed");
		}
	}
	aw_dev->rcv_test_volume_copy = vol_desc->ctl_volume;

	if (aw_dev->rcv_test_flag) {
		vol_desc->ctl_volume = aw_dev->rcv_test_volume_set;
	...
	aw_dev_sysctrl_mute_check_recovery(aw_dev);

	aw882xx_monitor_start(&aw_dev->monitor_desc);
	if(aw_dev->pilot_tone_threshold_update) {
		aw882xx_dsp_write_pilot_tone_threshold(aw_dev, aw_dev->pilot_tone_threshold_value);
	}
#ifdef AW_ALGO_AUTH_DSP
...
int aw882xx_device_stop(struct aw_device *aw_dev)
{
	...
	if (aw_dev->rcv_test_flag) {
		vol_desc->ctl_volume = aw_dev->rcv_test_volume_copy;
		compared_vol = AW_GET_MAX_VALUE(vol_desc->ctl_volume,
							vol_desc->monitor_volume);
		aw882xx_dev_set_volume(aw_dev, compared_vol);
		aw_dev->rcv_test_flag = 0;
	}
	...
	aw_dev_info(aw_dev->dev, "max_temp[%d], max_excu[%d], mute_count[%d]",
			aw_dev->bd_record.max_temp, aw_dev->bd_record.max_excu, aw_dev->mute_count);
	aw_dev_info(aw_dev->dev, "done");
	return 0;
}
