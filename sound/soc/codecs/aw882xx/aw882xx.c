...
#define AW882XX_DRIVER_VERSION "V1.15.0.31"
...
static unsigned int g_iv_output = false;
static bool g_get_bigdata = false;
static int g_last_irq_gpio = -1;
static int g_last_rst_gpio = -1;
static struct aw_init_data g_init_data;
...
void aw882xx_register_short_circuit_callback(int channel, void (*func)(int))
{
	int ret = -1;
	...
	list_for_each(pos, dev_list) {
		...
		if (local_dev->channel == channel)
			local_dev->ops.short_circuit_callback = func;
	}

}
EXPORT_SYMBOL_GPL(aw882xx_register_short_circuit_callback);

static void aw_short_circuit_report(struct aw882xx *aw882xx)
{
	aw_dev_err(aw882xx->dev, "aw882xx short circuit error");

	if (aw882xx->aw_pa->ops.short_circuit_callback) {
		aw882xx->aw_pa->ops.short_circuit_callback(aw882xx->aw_pa->channel);
	} else {
		aw_dev_err(aw882xx->dev, "short_circuit_callback is NULL");
		return;
	}
}

void aw882xx_register_i2c_error_callback(int channel, void (*func)(int))
{
	int ret = -1;
	struct list_head *dev_list = NULL;
	struct list_head *pos = NULL;
	...
	list_for_each (pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel == channel)
			local_dev->ops.i2c_callback = func;
	}

}
EXPORT_SYMBOL_GPL(aw882xx_register_i2c_error_callback);

static void aw_i2c_err_report(struct aw882xx *aw882xx, int err_type)
{
	aw_dev_err(aw882xx->dev, "aw882xx i2c rw error, err_type = %d", err_type);

	if (aw882xx->aw_pa->ops.i2c_callback) {
		aw882xx->aw_pa->ops.i2c_callback(aw882xx->aw_pa->channel);
	} else {
		aw_dev_err(aw882xx->dev, "i2c_callback is NULL");
		return;
	}
}
	...
	unsigned char reg_addr, unsigned int reg_data)
{
	...
	if (cnt) {
		aw882xx->aw_pa->bd_record.iic_abnormal_cnt += 1;
		aw_dev_info(aw882xx->dev, "iic_abnormal_cnt[%d]", aw882xx->aw_pa->bd_record.iic_abnormal_cnt);
	}
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		/*load cali te*/
		aw882xx_dev_init_cali_te(aw882xx->aw_pa);
	} else {
...
static int aw882xx_hw_params(struct snd_pcm_substream *substream,
			...
			struct snd_soc_dai *dai)
{
#if defined (AW_ABOX_PLATFORM) || defined (AW_SKT_ALGO_DTC)
	int ret = -1;
#endif
	aw_snd_soc_codec_t *codec = aw_get_codec(dai);
	struct aw882xx *aw882xx =
	...
	aw_dev_get_init_data(aw882xx->aw_pa, &g_init_data, g_aw882xx_dev_cnt);
	if (aw882xx->aw_pa->channel == g_aw882xx_dev_cnt - 1) {
#if 0
		ret = aw882xx_abox_sync_algo_param_load();
		if (ret < 0) {
			aw_dev_err(aw882xx->dev, "sync_algo_param_load failed");
		}
		aw_dev_info(aw882xx->dev, "sync_algo_param_load done");
#endif
#ifdef AW_SKT_ALGO_DTC
		ret = aw_dsp_recover_algo_packet((char *)(&g_init_data), sizeof(g_init_data));
		if (ret < 0) {
...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (!g_get_bigdata) {
			aw_big_data_update(aw882xx->aw_pa);
			g_get_bigdata = true;
		}
		...
		aw882xx->aw_pa->pstream = true;
		g_get_bigdata = false;
		aw_big_data_reset_st(aw882xx->aw_pa, false);
		mutex_lock(&aw882xx->lock);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	aw_dev_info(aw882xx->dev, "allow_pw[%s]", aw882xx->allow_pw ? "true" : "false");
	mutex_unlock(&aw882xx->lock);
...
static int aw882xx_rcv_test_volume_info(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_info *uinfo)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.kcontrol_codec(kcontrol);
...
static int aw882xx_rcv_test_volume_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.kcontrol_codec(kcontrol);
...
static int aw882xx_rcv_test_volume_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int value = 0;
	int compared_vol = 0;
	...
	if (aw882xx->pstream) {
		...
		aw882xx->aw_pa->rcv_test_volume_set = value;
	}
			...
			unsigned int size, unsigned int __user *tlv)
{
	...
	if (size != (sizeof(struct algo_auth_data) + AW_TLV_HEADER_SIZE)) {
		aw_dev_err(aw882xx->dev, "size[%d] != algo_auth_data[%lu] + tlv header size[%lu]",
			size, sizeof(struct algo_auth_data), AW_TLV_HEADER_SIZE);
		return -EINVAL;
	}
	...
	case SNDRV_CTL_TLV_OP_READ:
		...
		if (copy_to_user((void __user *)(tlv + AW_TLV_HEADER_NUM), (char *)&algo_data, sizeof(struct algo_auth_data)))
			ret = -EFAULT;
		break;
	case SNDRV_CTL_TLV_OP_WRITE:
		aw_dev_dbg(aw882xx->dev, "set algo auth data");
		if (copy_from_user(&algo_data, (void __user *)(tlv + AW_TLV_HEADER_NUM), sizeof(struct algo_auth_data)))
			ret = -EFAULT;
...
static int aw882xx_pilot_tone_threshold_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int value = 0;
	aw_snd_soc_codec_t *codec =
	...
	value = ucontrol->value.integer.value[0];
	if ((value > AW_PILOT_TONE_MAX_VALUE) || (value < 0)) {
		aw_dev_dbg(aw882xx->dev, "value over range\n");
		aw882xx->aw_pa->pilot_tone_threshold_update = 0;
		return 0;
	}
	aw_dev_info(aw882xx->dev, "ucontrol->value.integer.value[0]=%d", value);

	aw882xx->aw_pa->pilot_tone_threshold_value = value;
	if (aw882xx->pstream) {
		aw882xx_dsp_write_pilot_tone_threshold(aw882xx->aw_pa, value);
	}
	aw882xx->aw_pa->pilot_tone_threshold_update = 1;

	return 0;
}

static int aw882xx_pilot_tone_threshold_info(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_info *uinfo)
{
	/* set kcontrol info */
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = AW_PILOT_TONE_MAX_VALUE;

	return 0;
}

static int aw882xx_pilot_tone_threshold_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int value = 0;
	aw_snd_soc_codec_t *codec =
	...
	if (aw882xx->pstream) {
		aw882xx_dsp_read_pilot_tone_threshold(aw882xx->aw_pa, &value);
		ucontrol->value.integer.value[0] = value;
	} else {
		ucontrol->value.integer.value[0] = aw882xx->aw_pa->pilot_tone_threshold_value;
	}
...
static int aw882xx_hmute_info(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_info *uinfo)
{
	int count = 0;
	int ret = 0;
...
static int aw882xx_hmute_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.kcontrol_codec(kcontrol);
	...
	ucontrol->value.integer.value[0] = aw882xx->aw_pa->mute_flag;

	aw_dev_info(aw882xx->dev, "mute_flag[%s]",
		aw882xx->aw_pa->mute_flag ? "true" : "false");

	return 0;
}

static int aw882xx_hmute_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.kcontrol_codec(kcontrol);
	...
	if (ucontrol->value.integer.value[0])
		aw882xx->aw_pa->mute_flag = true;
	else
		aw882xx->aw_pa->mute_flag = false;

	if (aw882xx->pstream)
		aw882xx_dev_mute(aw882xx->aw_pa, aw882xx->aw_pa->mute_flag);
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_pilot_tone_threshold", aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_PILOT_TONE_THRESHOLD].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_PILOT_TONE_THRESHOLD].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw882xx_dev_control[KCTL_TYPE_PILOT_TONE_THRESHOLD].info = aw882xx_pilot_tone_threshold_info;
	aw882xx_dev_control[KCTL_TYPE_PILOT_TONE_THRESHOLD].put = aw882xx_pilot_tone_threshold_set;
	aw882xx_dev_control[KCTL_TYPE_PILOT_TONE_THRESHOLD].get = aw882xx_pilot_tone_threshold_get;

	kctl_name = devm_kzalloc(aw882xx->codec->dev, AW_NAME_BUF_MAX, GFP_KERNEL);
	if (!kctl_name)
		return -ENOMEM;

	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_rcv_volume", aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_RCV_TEST_VOLUME].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_RCV_TEST_VOLUME].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw882xx_dev_control[KCTL_TYPE_RCV_TEST_VOLUME].info = aw882xx_rcv_test_volume_info;
	aw882xx_dev_control[KCTL_TYPE_RCV_TEST_VOLUME].put = aw882xx_rcv_test_volume_put;
	aw882xx_dev_control[KCTL_TYPE_RCV_TEST_VOLUME].get = aw882xx_rcv_test_volume_get;

	kctl_name = devm_kzalloc(aw882xx->codec->dev, AW_NAME_BUF_MAX, GFP_KERNEL);
	if (!kctl_name)
		return -ENOMEM;
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_hmute", aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_HMUTE].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_HMUTE].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw882xx_dev_control[KCTL_TYPE_HMUTE].info = aw882xx_hmute_info;
	aw882xx_dev_control[KCTL_TYPE_HMUTE].put = aw882xx_hmute_set;
	aw882xx_dev_control[KCTL_TYPE_HMUTE].get = aw882xx_hmute_get;
...
static void aw882xx_interrupt_work(struct work_struct *work)
{
	...
	/*bit0|bit5*/
	if (reg_value & 0x0021) {
		aw_dev->bd_record.iis_abnormal_cnt += 1;
		aw_dev_info(aw882xx->dev, "iis_abnormal_cnt[%d]", aw_dev->bd_record.iis_abnormal_cnt);
	}

	/*bit3|bit11|bit12*/
	if (reg_value & 0x1808) {
		aw_dev->bd_record.short_circuit_cnt += 1;
		aw_dev_info(aw882xx->dev, "short_circuit_cnt[%d]", aw_dev->bd_record.short_circuit_cnt);
		aw_short_circuit_report(aw882xx);
	}
...
static int aw882xx_gpio_request(struct aw882xx *aw882xx)
{
	...
	if(aw882xx->reset_gpio == g_last_rst_gpio) {
		aw_dev_info(aw882xx->dev, "rst already request");
	} else {
	if (gpio_is_valid(aw882xx->reset_gpio)) {
		...
		if (ret) {
			...
			g_last_rst_gpio = aw882xx->reset_gpio;
		}
	}

	if (aw882xx->irq_gpio == g_last_irq_gpio) {
		aw_dev_info(aw882xx->dev, "int already request");
		return 0;
	}

	if (gpio_is_valid(aw882xx->irq_gpio)) {
		#ifdef AW_KERNEL_VER_OVER_6_12_0
		...
		g_last_irq_gpio = aw882xx->irq_gpio;
	}
		...
		struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "num-amp", &num_amp);
	if (ret) {
		num_amp = 2;
		aw_dev_info(aw882xx->dev,
			"read num-amp failed, default num-amp=%d", num_amp);
	} else {
		aw_dev_info(aw882xx->dev, "num_amp = %u", num_amp);
	}
	aw882xx->num_amp = num_amp;

	ret = of_property_read_u32(np, "chip-id", &chip_id);
	if (ret) {
		/*invalid dts chip id, use i2c chip id*/
		chip_id = 0;
		aw_dev_info(aw882xx->dev,
			"read chip-id failed, use chip id read from i2c");
	} else {
		aw_dev_info(aw882xx->dev, "chip_id = 0x%x", chip_id);
	}
	aw882xx->aw_pa->dts_chip_id = chip_id;
...
static void aw882xx_check_chipid(struct aw882xx *aw882xx)
{
	if (aw882xx->aw_pa->dts_chip_id != 0 &&
		aw882xx->aw_pa->chip_id != aw882xx->aw_pa->dts_chip_id) {
		aw_dev_err(aw882xx->dev, "i2c read chip_id[0x%x] dismatch dts read chip_id[0x%x], use dts chip_id",
			aw882xx->aw_pa->chip_id, aw882xx->aw_pa->dts_chip_id);
		aw882xx->aw_pa->chip_id = aw882xx->aw_pa->dts_chip_id;
	} else {
		aw_dev_info(aw882xx->dev, "i2c read chip_id match dts read chip_id or dts chip_id invalid, use i2c chip_id[0x%x]",
			aw882xx->aw_pa->chip_id);
	}
}

static irqreturn_t aw882xx_irq_handler(int irq, void *data)
{
...
static int aw882xx_interrupt_init(struct aw882xx *aw882xx)
{
	...
	if (gpio_is_valid(aw882xx->irq_gpio)) {
		...
		ret = devm_request_threaded_irq(aw882xx->dev,
					...
					aw882xx_irq_handler, aw882xx_irq, irq_flags,
					"aw882xx", aw882xx);
		if (ret != 0) {
...
static ssize_t surface_temp_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	...
	if (1 == sscanf(buf, "%d", &databuf[0])) {
		aw_dsp_write_surface_temperature(aw882xx->aw_pa->channel, databuf[0]);
	}
	aw_dev_info(aw882xx->dev, "set surface time %d Done", databuf[0]);

	return count;
}

static ssize_t surface_temp_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int surface_temperature = 0;
	struct aw882xx *aw882xx = dev_get_drvdata(dev);

	aw_dsp_read_surface_temperature(aw882xx->aw_pa->channel, &surface_temperature);
...
static ssize_t voltage_offset_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	int databuf[2] = {0};
	...
	if (2 == sscanf(buf, "%d %d", &databuf[0], &databuf[1])) {
		if (databuf[0]) {
			aw882xx_dsp_write_voltage_offset(aw882xx->aw_pa, databuf[1]);
		}
...
static ssize_t voltage_offset_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int offset = 0;
	...
	aw882xx_dsp_read_voltage_offset(aw882xx->aw_pa, &offset);
	...
	struct device_attribute *attr, char *buf)
{
	...
	te[0] = aw_dsp_read_cur_te(0);
	te[1] = aw_dsp_read_cur_te(1);
	aw_dev_info(aw882xx->dev, "te[%d][%d]", te[0], te[1]);
...
static DEVICE_ATTR_RW(surface_temp);
static DEVICE_ATTR_RW(voltage_offset);
static DEVICE_ATTR_RO(cur_temp);
...
static struct attribute *aw882xx_attributes[] = {
	...
	&dev_attr_surface_temp.attr,
	&dev_attr_voltage_offset.attr,
...
#ifdef AW_KERNEL_VER_OVER_6_6_18
